"""OSM Large HCCI Benchmark: viewport queries comparing HCCI vs GiST on US OSM data.

Runs class-filtered viewport queries on the large OSM dataset (10M+ rows,
27 US metros) to validate HCCI performance on geographic feature data with
diverse categorical labels (building, restaurant, school, etc.).

Usage:
    python -m benchmarks.osm_large_hcci_benchmark
    python -m benchmarks.osm_large_hcci_benchmark --trials 500 --viewport-frac 0.05
"""
...
TABLE = "osm_large"
DATASET_ID = "osm_us"
HCCI_INDEX = "idx_osm_large_hcci_covering"


def load_osm_large_metadata():
    """Load OSM large dataset metadata."""
...
def build_gist_query(class_label: str, viewport: tuple) -> str:
...
def run_benchmark(
    ...
    seed: int = config.RANDOM_SEED + 12000,
) -> dict:
    """Run HCCI vs GiST viewport benchmark on OSM large data."""
    bounds = meta["bounds"]
    ...
    # Get top class labels by frequency (skip very rare ones)
...
def run_io_decomposition(
    ...
    n_trials: int = 50,
    viewport_frac: float = 0.05,
    seed: int = config.RANDOM_SEED + 12500,
) -> dict:
    ...
    class_labels = list(class_enum.keys())[:20]
    rng = np.random.RandomState(seed)
...
def main():
    parser = argparse.ArgumentParser(description="OSM Large HCCI Benchmark")
    ...
    print("=" * 60)
    print("  OSM Large HCCI Benchmark (27 US Metros)")
    print(f"  Trials: {args.trials}, Viewport: {args.viewport_frac}")
    print("=" * 60)

    conn = psycopg2.connect(config.dsn())
    meta = load_osm_large_metadata()
    ...
    path = save_results(all_results, "osm_large_hcci_benchmark")
    print(f"\n{'='*60}")
